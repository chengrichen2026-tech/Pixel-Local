...
import { IconChevronDown, IconChevronRight, IconDeviceFloppy, IconDownload, IconEye, IconEyeOff, IconFile, IconFolder, IconFolderOpen, IconFrame, IconPhoto, IconPlus, IconRefresh, IconSquare, IconTrash, IconTypography } from "@tabler/icons-react";

type EditorPage = { id: string; name: string; hidden?: boolean };
type ObjectMeta = FabricObject & { id?: string; name?: string; kind?: string; frameId?: string; pageId?: string; hidden?: boolean; nameFollowsText?: boolean; gridPreset?: string; lastLeft?: number; lastTop?: number };
...
type FrameClipboard = { frame: ObjectMeta; children: ObjectMeta[] };
...
const FRAME_PRESETS = [{ label: "方形", width: 800, height: 800 }, { label: "竖版 3:4", width: 750, height: 1000 }, { label: "竖屏 9:16", width: 1080, height: 1920 }, { label: "横屏 16:9", width: 1920, height: 1080 }];
...
const isTransientSerializedObject = (value: SerializedFabricValue) => {
  ...
  return type === "activeselection" || kind === "activeselection" || kind === "frame-label" || kind === "grid-overlay" || kind === "drawing-preview";
};
...
export default function Home() {
  ...
  const isTransientObject = (object: ObjectMeta) => {
    ...
    return type === "activeselection" || kind === "activeselection" || kind === "frame-label" || kind === "frame-selection-outline" || kind === "grid-overlay" || kind === "drawing-preview";
  };
  const serialize = () => JSON.stringify(cleanSerializedCanvasValue(editor.current?.toObject(["id", "name", "kind", "frameId", "pageId", "hidden", "lastLeft", "lastTop"])));
  ...
  const applyObjectSelectionStyle = (object: FabricObject) => object.set({ borderColor: "#4099ff", cornerColor: "#ffffff", cornerStrokeColor: "#4099ff", transparentCorners: false });
  ...
  const applyActivePageVisibility = () => {
    ...
    const activeFrameId = active?.kind === "frame" ? active.id : active?.frameId;
    canvas.getObjects().forEach((item) => {
      ...
      const childEnabled = !meta.frameId || meta.frameId === activeFrameId;
  ...
  const copySelectedFrame = () => {
    ...
    const frame = active instanceof ActiveSelection
      ? active.getObjects().find((item) => (item as ObjectMeta).kind === "frame") as ObjectMeta | undefined
      : active.kind === "frame" ? active : undefined;
    if (!frame?.id) { setSavedState("请先选中要复制的 Frame"); return; }
    const children = canvas.getObjects().filter((item) => {
      const meta = item as ObjectMeta;
    ...
    frameClipboard.current = { frame, children };
    setSavedState(`已复制 ${frame.name || "Frame"}，可切换页面粘贴`);
  };
  ...
  const pasteCopiedFrame = async () => {
    ...
    try {
      const sourceFrame = clipboard.frame;
      const frame = await cloneCanvasObject(sourceFrame);
      const children = await Promise.all(clipboard.children.map(async (source) => {
        const clone = await cloneCanvasObject(source);
        Object.assign(clone, { name: source.name, kind: source.kind, nameFollowsText: source.nameFollowsText });
        return clone;
      }));
      const existingFrames = frames();
      const left = existingFrames.length
        ? Math.max(...existingFrames.map((item) => item.left + frameDesignWidth(item))) + 80
        : sourceFrame.left;
      const top = existingFrames.length ? existingFrames.reduce((best, item) => item.left > best.left ? item : best).top : sourceFrame.top;
      ...
      children.forEach((child) => {
        resetIdentity(child); child.set({ left: child.left + dx, top: child.top + dy }); applyObjectSelectionStyle(child);
        if (child instanceof Rect && child.kind === "rectangle") attachRectangleRadiusControl(child as Rect & ObjectMeta);
        if (child instanceof Group) syncGroupMemberFrame(child as Group & ObjectMeta, frame);
        applyFrameClip(child, frame); child.setCoords(); canvas.add(child);
      });
      canvas.setActiveObject(frame); canvas.requestRenderAll(); commit();
      setSavedState(`已粘贴到 ${pagesRef.current.find((page) => page.id === activePageIdRef.current)?.name || "当前页面"}`);
    } catch (error) {
  ...
  const syncFrameChildren = (frame: ObjectMeta, dx = 0, dy = 0) => {
    editor.current?.getObjects().forEach((item) => {
      ...
      if (dx || dy) child.set({ left: child.left + dx, top: child.top + dy });
  ...
  const attachRectangleRadiusControl = (rectangle: Rect & ObjectMeta) => {
    ...
    corners.forEach((corner) => {
      ...
      rectangle.controls[`cornerRadius${corner}`] = new Control({
        ...
        render: (ctx, left, top) => {
          ctx.save(); ctx.translate(left, top); ctx.fillStyle = "#ffffff"; ctx.strokeStyle = "#1296f3"; ctx.lineWidth = 1.25;
  ...
  const refresh = () => {
    ...
    setSelected(active ? {
      ...
      cornerRadius: active instanceof Rect && active.kind === "rectangle" ? Number(active.rx || 0) : undefined,
  ...
  const selectLayerFromSidebar = (layer: ObjectMeta, shiftKey: boolean) => {
    ...
    const sameFrame = current.length > 0 && current.every((object) => object.kind !== "frame" && object.frameId === layer.frameId);
  ...
  useEffect(() => {
    ...
    const canvas = new Canvas(htmlCanvas.current, {
      ...
      selectionColor: "rgba(67,97,238,.12)", selectionBorderColor: "#4361ee",
    });
    ...
    const afterChange = ({ target }: { target?: FabricObject }) => {
      ...
      if (changed?.kind === "frame") {
        ...
        if (destination) {
          ...
          if (changed instanceof Group && changed.kind === "group") syncGroupMemberFrame(changed as Group & ObjectMeta, destination);
        } else {
          changed.frameId = undefined; changed.clipPath = undefined; changed.set({ dirty: true });
          if (changed instanceof Group && changed.kind === "group") syncGroupMemberFrame(changed as Group & ObjectMeta);
        }
      }
      commit();
    };
    ...
    canvas.on("selection:created", refreshSelection); canvas.on("selection:updated", refreshSelection); canvas.on("selection:cleared", () => { canvas.uniformScaling = true; applyActivePageVisibility(); syncFrameSelectionOutlines(); canvas.requestRenderAll(); refresh(); });
    ...
    if (saved) {
      ...
      canvas.loadFromJSON(cleanSaved).then(() => {
        canvas.backgroundColor = "#e7e6e3"; cleanLoadedCanvas(canvas); showPage(activePageIdRef.current, false); canvas.requestRenderAll(); history.current = [cleanSaved]; historyIndex.current = 0; restoring.current = false; commit();
      });
    } else {
      restoring.current = true;
      ...
      if (legacy) {
        canvas.loadFromJSON(sanitizeProjectJson(legacy)).then(() => {
          ...
          canvas.add(frame); canvas.sendObjectToBack(frame); canvas.backgroundColor = "#e7e6e3"; restoring.current = false; commit();
        });
      } else {
  ...
  const updateSelected = (patch: Record<string, unknown>) => { const active = editor.current?.getActiveObject() as ObjectMeta | undefined; if (!active) return; if (active instanceof Textbox && ("stroke" in patch || "strokeWidth" in patch)) patch.paintFirst = "stroke"; active.set(patch); active.setCoords(); if (active.kind === "frame") syncFrameLabel(active); editor.current?.requestRenderAll(); commit(); };
  ...
  const updateObjectDimension = (dimension: "width" | "height", rawValue: string, input: HTMLInputElement) => {
    ...
    if (!object) return;
    const minimum = object instanceof Rect && object.kind === "rectangle" ? Math.max(1, Math.round(object.rx) * 2) : 40;
    ...
    if (object.kind === "frame" || object.kind === "rectangle") object.set({ [dimension]: Math.round(value), scaleX: 1, scaleY: 1 });
    ...
    if (object instanceof Rect && object.kind === "rectangle") { const radius = Math.min(object.rx, object.width / 2, object.height / 2); object.set({ rx: radius, ry: radius }); }
  ...
  const updateObjectPosition = (axis: "x" | "y", rawValue: string, input: HTMLInputElement) => {
    ...
    object.setCoords(); if (frame) applyFrameClip(object, frame); editor.current?.requestRenderAll(); commit();
  };
  const updateRectangleRadius = (rawValue: string) => {
    const object = editor.current?.getActiveObject();
    if (!(object instanceof Rect) || (object as ObjectMeta).kind !== "rectangle") return;
    const radius = Math.max(0, Math.min(Math.round(Number(rawValue) || 0), Math.floor(Math.min(object.width, object.height) / 2)));
    object.set({ rx: radius, ry: radius, dirty: true }); object.setCoords(); editor.current?.requestRenderAll(); refresh(); commit();
  };
  ...
  const executePixelLocalCommands = async (input: PixelLocalCommand | PixelLocalCommand[]): Promise<PixelLocalCommandResult> => {
    ...
    try {
      ...
      canvas.backgroundColor = "#e7e6e3"; cleanLoadedCanvas(canvas); showPage(beforeActivePageId, false); canvas.requestRenderAll(); restoring.current = false; refresh();
  ...
  const renderAlignmentControls = () => <div className="alignment-panel"><h3>对齐</h3><div className="alignment-grid">{([['left','左'],['center','水平中'],['right','右'],['top','上'],['middle','垂直中'],['bottom','下']] as [ObjectAlignment,string][]).map(([value,label]) => <button key={value} onClick={() => alignSelectedObjects(value)}>{label}</button>)}</div><div className="inspector-actions"><button onClick={groupSelectedObjects}>编组</button></div></div>;
  const renderSelectedProperties = () => selected && <div className="properties">
    ...
    {selected.text !== undefined && <>
      ...
      <label>文字对齐<select value={selected.textAlign || 'left'} onChange={(e) => updateSelected({ textAlign: e.target.value })}><option value="left">左对齐</option><option value="center">居中</option><option value="right">右对齐</option></select></label>
    ...
    <label>透明度<input type="range" min="0.05" max="1" step="0.05" value={selected.opacity} onChange={(e) => updateSelected({ opacity: Number(e.target.value) })} /></label>
  ...
  const renderLayerTreeRow = (layer: FabricObject, index: number) => {
    ...
    const isSelected = editor.current?.getActiveObjects().includes(layer);
    if (layer instanceof Group && meta.kind === "group") {
      ...
      return <div key={meta.id || `${meta.name}${index}`} className={`layer-row group-row ${meta.hidden ? "is-hidden" : ""} ${isSelected ? "active" : ""}`}>
        ...
        <button className="layer-main" onClick={(e) => selectLayerFromSidebar(meta, e.shiftKey)} onDoubleClick={() => startRenameLayer(meta)} title="双击重命名；Shift 点击多选">{expanded ? <IconFolderOpen size={15} className="group-folder-icon" /> : <IconFolder size={15} className="group-folder-icon" />}<span className="layer-name">{meta.name || "编组"}</span></button>
        ...
        {expanded && <div className="group-children">{children.map((child, childIndex) => <div className={`group-child-row ${(child as ObjectMeta).hidden ? "is-hidden" : ""}`} key={(child as ObjectMeta).id || childIndex}>
    ...
    <section className="workspace">
      <aside className="left-panel panel">
        ...
        <section className="sidebar-block layers-block">
          ...
          <div className="layer-tree">{layers.some((layer) => { const meta = layer as ObjectMeta; return meta.kind !== "frame" && !meta.frameId; }) && <div className="outside-layers"><div className="outside-layers-title">画布外</div>{layers.filter((layer) => { const meta = layer as ObjectMeta; return meta.kind !== "frame" && !meta.frameId; }).map(renderLayerTreeRow)}</div>}{frames().length === 0 && <p className="empty layer-empty">当前页面还没有 Frame。</p>}{frames().map((frame) => <div className="frame-node" key={frame.id}><div className={`layer-row frame-row ${frame.hidden ? "is-hidden" : ""}`}><button className="frame-toggle" onClick={(e) => { e.stopPropagation(); const id = String(frame.id); const next = new Set(collapsedFrameIdsRef.current); if (next.has(id)) next.delete(id); else next.add(id); updateCollapsedFrameIds(next); }} aria-label={`${collapsedFrameIds.has(String(frame.id)) ? "展开" : "折叠"}${frame.name || "Frame"}`} title={collapsedFrameIds.has(String(frame.id)) ? "展开 Frame" : "折叠 Frame"}>{collapsedFrameIds.has(String(frame.id)) ? <IconChevronRight size={14} /> : <IconChevronDown size={14} />}</button>{editingLayerId === frame.id ? <input autoFocus className="layer-rename" value={layerNameDraft} onChange={(e) => setLayerNameDraft(e.target.value)} onBlur={() => finishRenameLayer(frame)} onKeyDown={(e) => { if (e.key === "Enter") e.currentTarget.blur(); if (e.key === "Escape") setEditingLayerId(null); }} /> : <><button className={`layer-main ${editor.current?.getActiveObject() === frame ? "active" : ""}`} onClick={() => { editor.current?.setActiveObject(frame); editor.current?.requestRenderAll(); refresh(); }} onDoubleClick={() => startRenameLayer(frame)} title="双击重命名"><IconFrame size={15} className="frame-icon" /><span className="layer-name">{frame.name}</span></button><button className="visibility-toggle layer-visibility" onClick={(e) => { e.stopPropagation(); toggleLayerVisibility(frame); }} aria-label={`${frame.hidden ? "显示" : "隐藏"}${frame.name || "Frame"}`} title={frame.hidden ? "显示图层" : "隐藏图层"}>{frame.hidden ? <IconEyeOff size={13} /> : <IconEye size={13} />}</button><button className="delete-layer" onClick={(e) => { e.stopPropagation(); removeLayerObject(frame); }} aria-label={`删除${frame.name || "Frame"}`} title="删除"><IconTrash size={13} /></button></>}</div>{!collapsedFrameIds.has(String(frame.id)) && <div className="frame-children">{layers.filter((layer) => (layer as ObjectMeta).frameId === frame.id).map(renderLayerTreeRow)}</div>}</div>)}</div>
        </section>
      </aside>
      <section className="canvas-area" ref={canvasHost}>
        ...
        <div className="editor-toolbar" role="toolbar" aria-label="编辑工具">
          ...
          <select aria-label="Frame 尺寸预设" defaultValue="" onChange={(e) => { const preset = FRAME_PRESETS[Number(e.target.value)]; if (preset) addFrame(preset.width, preset.height, preset.label); e.target.value = ""; }}><option value="" disabled>尺寸</option>{FRAME_PRESETS.map((preset, index) => <option value={index} key={preset.label}>{preset.label} {preset.width}×{preset.height}</option>)}</select>
          <span className="toolbar-divider" />
